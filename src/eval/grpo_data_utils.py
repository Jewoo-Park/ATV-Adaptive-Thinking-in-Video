...
def normalize_answer(answer_or_solution: str) -> str | None:
    letter = normalize_gt_letter(str(answer_or_solution or ""))
    if letter is None:
        return None
...
def format_options(options: object) -> list[str]:
    if isinstance(options, list):
        ...
        for idx, opt in enumerate(options):
            ...
            if not text:
                continue
            if OPTION_LABEL_RE.match(text):
                out.append(text)
            elif idx < 10:
                out.append(f"{chr(ord('A') + idx)}. {text}")
        return out
